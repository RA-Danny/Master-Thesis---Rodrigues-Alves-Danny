...
1. Why ? Motivation?
    Flaw of previous model is that they are not really account generic. Of course there is some account-specific features, but those are not enough to catch some use case. For example, if an account is used to make a big order in June, then a small order in September and ordering in the same fashion each 15 months, the model will most probably not able to catch some behavior. That's the main motivation for using Long-Short Term Memory (LSTM) networks in this project. Part of the Recurrent Networks family, LSTMs are able to retain such connection with a specified amount of time.

2. Data reshape
    Previously, the neural network was expecting a two-dimensions inputs, but LSTM only deals with three-dimensionalities input: \texttt{[batch size, step, features]}. There is a need to reshape the data into one entry per account. That exactly what has been done: all entries related to the same account have been sorted by the (month,year) they are related to and combined in such way.

3. Results
    Trained witht the features giving best predictions for model from previous section. Better at first, but not in the fine-tuning phase.
