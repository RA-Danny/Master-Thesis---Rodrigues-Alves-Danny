...
\iffalse
            \begin{table}[]
                ...
                J        & 0       & 0             & 2.5     & 2            
                \end{tabular}
                \caption{Example of output truth and predictions}
                \label{fig:1}
            
                \offinterlineskip
                \moveright 5cm \hbox{\raisebox{1.8cm}[10pt][10pt]{\rotatebox[origin=c]{90}{\parbox[c][0pt][c]{15cm}{\textbf{True class}\\[50pt]}}}\par}
                \hspace*{1cm}\MyHBox[\dimexpr4.1cm+1\fboxsep\relax]{Predicted class}\par
                \hspace*{1cm}\MyHBox{0}\MyHBox{1}\MyHBox{2}\MyHBox{3+}\par
                \MyTBox{0}{2}{0}{1}{0}
                \MyTBox{1}{1}{1}{0}{0}
                \MyTBox{2}{0}{1}{0}{1}
                \MyTBox{3+}{1}{0}{1}{1}
                \caption{Confusion matrix for Table 3.1}
                \label{fig:my_label}
            \end{table}
\fi

\begin{center}
  \makebox[\textwidth]{\includegraphics[width=15cm]{images/caca.png}}
\end{center}
...
Now that the machine learning parameters have been determined, the data to use with the model must be detailed. Data comes from \textit{Company A}'s CRM, a Dynamics 365 Online system. Thanks to the Web Api offered by Dynamics 365, all entities of the CRM have been retrieved and analysis. For the analysis of data and machine learning experimentations, the development has been done in \textit{Jupyter Notebooks} with a \texttt{Python 3} kernel.
